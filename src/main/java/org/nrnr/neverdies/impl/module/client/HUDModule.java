...
public class HUDModule extends ToggleModule {
    ...
    Config<Boolean> watermark2Config = new BooleanConfig("Watermark 2", "Displays client name and version watermark, instead it shows: Neverdies.me", false);
    //Config<Boolean> welcomer = new BooleanConfig("Welcomer", "Welcomes you.", false);
    ...
    private final Animation chatOpenAnimation = new Animation(false, 50L, Easing.LINEAR);
        ...
        if (mc.player != null && mc.world != null) {
            ...
            if (watermarkConfig.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("%s %s%s%s",
                        NeverdiesMod.MOD_NAME, NeverdiesMod.MOD_VER,
                        "-extern+345f87c02373f", "" + ""), 2.0f, topLeft, getHudColor(rainbowOffset));
                // topLeft += 9.0f;
            }
            if (watermark2Config.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("%s %s%s%s",
                        "Neverdies.me", NeverdiesMod.MOD_VER,
                        "-extern+345f87c02373f", "" + ""), 2.0f, topLeft, getHudColor(rainbowOffset));
            }

            /*if (welcomer.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("%s %s%s%s",
                        "Welcome to Neverdies.me :')"), -5.0f, topLeft, getHudColor(rainbowOffset));
            }*/
            ...
            if (potionEffectsConfig.getValue()) {
                for (StatusEffectInstance e : mc.player.getStatusEffects()) {
                    ...
                    int width = RenderManager.textWidth(text);
                    RenderManager.renderText(event.getContext(), text,
                            res.getScaledWidth() - width - 1.0f, renderingUp ? bottomRight : topRight, getHudColor(rainbowOffset));
                    if (renderingUp) {
                        bottomRight -= 9.0f;
                    } else {
                        topRight += 9.0f;
