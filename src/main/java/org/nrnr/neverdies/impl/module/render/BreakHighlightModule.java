    ...
    public void onRenderWorld(RenderWorldEvent event) {
        ...
        if (modeConfig.getValue() == HighlightMode.NORMAL) {
            ...
            for (Int2ObjectMap.Entry<BlockBreakingInfo> info :
                ...
                RenderManagerWorld.renderBox(event.getMatrices(), new Box(x - sizeX,
                        y - sizeY, z - sizeZ, x + sizeX, y + sizeY, z + sizeZ), Modules.COLORS.getRGB(60));
                RenderManagerWorld.renderBoundingBox(event.getMatrices(), new Box(x - sizeX,
            ...
            for (Map.Entry<BlockBreakingProgressS2CPacket, Long> mine : breakingProgress.entrySet()) {
                ...
                RenderManagerWorld.renderBox(event.getMatrices(), scaled, ((ColorConfig) colorConfig).getValue(60).getRGB());
                RenderManagerWorld.renderBoundingBox(event.getMatrices(), scaled, 1.5f, ((ColorConfig) colorConfig).getValue(125).getRGB());
            }
        }
    }
