...
public class BedAuraModule extends RotationModule {
    ...
    Config<Boolean> strictDirectionConfig = new BooleanConfig("StrictDirection", "Interacts with oget nly visible directions when placing crystals", true);
    ...
    public void onRenderWorld(RenderWorldEvent event){
        ...
        if (!(renderpos == null)) {
            //RenderManagerWorld.renderBoundingBox(event.getMatrices(), renderpos, 1.0f, Modules.COLORS.getRGB());
            RenderManagerWorld.renderBox(event.getMatrices(), renderpos,
                   Modules.COLORS.getRGB(70));
            RenderManagerWorld.renderBoundingBox(event.getMatrices(), renderpos,
