...
public class BlinkModule extends ToggleModule {
    ...
    public void onRenderOverlayPost(RenderOverlayEvent.Post event) {
        if (renderConfig.getValue()){
            ...
            matrixStack.push();
            RenderManagerWorld.renderText(event.getContext(),"You are Currently Blinked", ((screenWidth / 2) - (client.textRenderer.getWidth("You are Currently Blinked") / 2)),
                    (screenHeight / 2) - (client.textRenderer.fontHeight / 2) + 25, Modules.COLORS.getRGB());
            RenderManagerWorld.renderLine(event.getContext().getMatrices(), mc.player.getX(), mc.player.getY(), mc.player.getZ(), serverModel.getX(), serverModel.getY(), serverModel.getZ(), 15f, Modules.COLORS.getRGB());
