    ...
    public void onRenderWorld(RenderWorldEvent event) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (direction == Direction.EAST && dx >= 0.65) {
                RenderManagerWorld.drawLine(event.getMatrices(), x, y, z, x, y, z + 1.0, color.getRGB());
            } else if (direction == Direction.WEST && dx <= 0.35) {
                RenderManagerWorld.drawLine(event.getMatrices(), x + 1.0, y, z, x + 1.0, y, z + 1.0, color.getRGB());
            } else if (direction == Direction.SOUTH && dz >= 0.65) {
                RenderManagerWorld.drawLine(event.getMatrices(), x, y, z, x + 1.0, y, z, color.getRGB());
            } else if (direction == Direction.NORTH && dz <= 0.35) {
                RenderManagerWorld.drawLine(event.getMatrices(), x, y, z + 1.0, x + 1.0, y, z + 1.0, color.getRGB());
            }
        }
    }
}
