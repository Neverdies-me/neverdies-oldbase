...
import org.nrnr.neverdies.impl.manager.player.HoldingManager;
...
public class InventoryUtil implements Globals {
    ...
    public static boolean pickSwitch(int slot) {
        if (slot >= 0) {
            HoldingManager.modifyStartTime = System.currentTimeMillis();
            pickSlot = slot;
    ...
    public static boolean invSwitch(int slot) {
        if (slot >= 0) {
            ...
            mc.getNetworkHandler().sendPacket(new ClickSlotC2SPacket(handler.syncId,
                    handler.getRevision(), PlayerInventory.MAIN_SIZE + HoldingManager.slot,
                    slot, SlotActionType.SWAP, handler.getSlot(slot).getStack(), stack)
            );
            slots = new int[]{slot, HoldingManager.slot};
            return true;
        }
        return false;
    }
    ...
    public static boolean testInHotbar(Predicate<ItemStack> predicate) {
        ...
        for (int i = FindItemResult.HOTBAR_START; i < FindItemResult.HOTBAR_END; i++) {
    ...
    public static FindItemResult findInHotbar(Predicate<ItemStack> isGood) {
        if (testInOffHand(isGood)) {
            return new FindItemResult(FindItemResult.OFFHAND, mc.player.getOffHandStack().getCount());
        }
    ...
    public static boolean swap(int slot, boolean swapBack) {
        if (slot == FindItemResult.OFFHAND) return true;
    ...
    public static class Action {
        ...
        public static boolean isHotbar(int i) {
            return i >= FindItemResult.HOTBAR_START && i <= FindItemResult.HOTBAR_END;
        }

        public static boolean isMain(int i) {
            return i >= FindItemResult.MAIN_START && i <= FindItemResult.MAIN_END;
        }

        public static boolean isArmor(int i) {
            return i >= FindItemResult.ARMOR_START && i <= FindItemResult.ARMOR_END;
        }

        private static int indexToId(int i) {
        ...
        public Action fromHotbar(int i) {
            return from(FindItemResult.HOTBAR_START + i);
        }

        public Action fromOffhand() {
            return from(FindItemResult.OFFHAND);
        }

        public Action fromMain(int i) {
            return from(FindItemResult.MAIN_START + i);
        }

        public Action fromArmor(int i) {
            return from(FindItemResult.ARMOR_START + (3 - i));
        }
        ...
        public void toHotbar(int i) {
            to(FindItemResult.HOTBAR_START + i);
        }

        public void toOffhand() {
            to(FindItemResult.OFFHAND);
        }

        public void toMain(int i) {
            to(FindItemResult.MAIN_START + i);
        }

        public void toArmor(int i) {
            to(FindItemResult.ARMOR_START + (3 - i));
        }
        ...
        public void slotHotbar(int i) {
            slot(FindItemResult.HOTBAR_START + i);
        }

        public void slotOffhand() {
            slot(FindItemResult.OFFHAND);
        }

        public void slotMain(int i) {
            slot(FindItemResult.MAIN_START + i);
        }

        public void slotArmor(int i) {
            slot(FindItemResult.ARMOR_START + (3 - i));
        }
